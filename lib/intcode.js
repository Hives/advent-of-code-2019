...
const intcode = (program, inputsArray = []) => {
    let counter = 0;
    let pointer = 0;

    const inputs = inputsArray.slice().reverse()
    const output = [];
    ...
    while (data[pointer] !== 99) {
        ...
        switch (instruction.opcode) {
            ...
            case '3':
                ...
                if (input === undefined) {
                    return { data: data, output: output, status: 'MISSING_INPUT' }
                }
    ...
    return { data: data, output: output, status: 'COMPLETE' }
}
