...
test('multiple inputs', () => {
    const program = [3, 5, 3, 6, 99, 0, 0]
    expect(intcode(program, [42, 43]).data[5]).toBe(42)
    expect(intcode(program, [42, 43]).data[6]).toBe(43)
})

test('status code "COMPLETE"', () => {
    expect(intcode([1,0,0,0,99]).status).toBe('COMPLETE')
})

test('status code "MISSING_INPUT"', () => {
    const program = [3, 5, 3, 6, 99, 0, 0]
    expect(intcode(program, [42]).status).toBe('MISSING_INPUT')
})
