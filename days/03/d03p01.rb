...
def render_wire(wire, origin, field, marker)
...
def render(input)
  ...
  render_wire(input[0], origin, field, '1')
  render_wire(input[1], origin, field, '2')
...
def get_crosses(field)
  ...
  field.each_with_index do |row, j|
    row.each_with_index do |cell, i|
      crosses << [i, j] if cell == 'X'
    end
  end
  crosses
end

def get_distance_from_origin(point, origin)
  (point[0] - origin[0]).abs + (point[1] - origin[1]).abs
end

def closest_distance_from_origin(points, origin)
  points.map { |point| get_distance_from_origin(point, origin) }.min
end
